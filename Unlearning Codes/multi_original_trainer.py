...
from dataset.dataset import CelebAMultiClassifiation
    ...
    if args.training:
        ...
        for e in range(args.epoch):
            with torch.set_grad_enabled(True):
                train_loss, train_accuracies = train_one_epoch(args=args,
                                                               epoch=e,
                                                               device=device,
                                                               model=model,
                                                               loader=train_loader,
            ...
            for i in range(len(args.task)):
                logs["Train " + args.task[i]] = np.mean(train_accuracies[i])
        ...
        pretrained_adversary_network_path = 'checkpoints/original_multi/Mouth_Slightly_Open_Pointy_Nose/Mouth_Slightly_Open_Pointy_Nose_final.pth'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--wandb', default=True, action='store_true', help='Use wandb for lpython mul   ogging')
    parser.add_argument('--group_name', type=str, default='Multi Classification Training Process', choices=('none', 'cosinewarmup'))

    parser.add_argument('--task', type=list, default=['Mouth_Slightly_Open','Pointy_Nose'], help='Attributes')
    #parser.add_argument('--task', type=list, default=['Mouth_Slightly_Open', 'Bald'], help='Attributes')
    #parser.add_argument('--task', type=list, default=['Mouth_Slightly_Open', 'Smiling'], help='Attributes')
