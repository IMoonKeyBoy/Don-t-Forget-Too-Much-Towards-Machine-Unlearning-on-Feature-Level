...
def training(args):
    ...
    if args.is_training_process:
        ...
        pretrained_adversary_network_path = args.save_path + "/"+args.task_labels[0] + "_final.pth"
        print(pretrained_adversary_network_path)
        model = torch.load(pretrained_adversary_network_path)
        with torch.no_grad():
            valid_loss, valid_accuracies = valid_one_epoch(
                                                           epoch=0,
                                                           device=device,
                                                           model=model,
                                                           loader=valid_loader)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--is_training_process', default=False, action='store_true', help='Use wandb for logging')
