...
save_path = './icCNN/basic_fmap/resnet_iccnn_celeba_original/'  # for get_feature
acc_path = './icCNN/basic_fmap/resnet_iccnn_celeba_original/acc/'

log_path = './icCNN/resnet/'
dataset = '%s_resnet_%s_iccnn_original' % (LAYERS, DATANAME)
log_path = log_path + dataset + '/'
pretrain_model = "/home/hengxu/Data/PycharmWorkspace/Technical Paper 3/icCNN-main-new/icCNN/resnet/18_resnet_celeb_iccnn_original/model_2400.pth"
...
def get_Data_without_trans(is_train, dataset_name, batch_size):
...
def net_train():
    ...
    net = None
    if LAYERS == '18':
        net = ResNet18(num_class=NUM_CLASSES, pretrained=False).cuda()
    elif LAYERS == '50':
        net = ResNet50(num_class=NUM_CLASSES, pretrained=False).cuda()
    ...
    for epoch in range(EPOCH + 1):
        if epoch % T == 0 and epoch < STOP_CLUSTERING:
            ...
            if epoch % 100 == 0 or epoch == 1:
                acc = test_celeb(net, testset_loader, NUM_CLASSES)
...
def get_feature():
    _, testset_test = get_Data(True, DATANAME, BATCHSIZE)
    _, testset_feature = get_Data(False, DATANAME, BATCHSIZE)
    _, testset_feature_without_trans = get_Data_without_trans(False, DATANAME, BATCHSIZE)
    ...
    # original_image = iter(testset_feature_without_trans)

    for batch_step, input_data in tqdm(enumerate(testset_feature, 0), total=len(testset_feature), smoothing=0.9):
        inputs, labels = input_data
        inputs, labels = inputs.to(device), labels.to(device)

        # inputs_without, labels__without = next(original_image)
        # inputs_without, labels__without = inputs_without.to(device), labels__without.to(device)

        net.eval()
        f_map = net(inputs, eval=True)

        # f_map = channel_max_min_whole(f_map.cpu().detach().numpy())

        f_map = f_map.cpu().detach().numpy()
        imagenumpy = []

        if batch_step == 0:
            index_number = 3
            for index_channel in save_channel:  # range(10):
                fig = f_map[index_number][index_channel]
                fig = cv.resize(fig, (112, 112)) * 255.0
                img = to_pil_image(inputs[index_number])
                img = addTransparency(img, 0.5)
                ...
                for i in range(zeros_mask.shape[0]):
                    for j in range(zeros_mask.shape[1]):
                        if np.sum((zeros_mask[i][j] / 255.0) > 0.2):  # vgg/cub 0.5 # VOC animal 0.5
                            mask_for_red[i][j] = 1
                            mask_img[i][j] = 0  # ori_img[i][j] here is mask image
                        else:
                            mask_for_red[i][j] = 0
                imagenumpy.append(mask_img)

            columns = int(len(imagenumpy) ** 0.5)
            rows = int(len(imagenumpy) ** 0.5)
            fig = plt.figure(figsize=(columns, rows))
            image_index = 1
            for i in trange(columns * columns):
                fig.add_subplot(rows, columns, image_index)
                image_index = image_index + 1
                plt.axis('off')
                plt.tight_layout()
                plt.title(save_channel_title[i - 1])
                plt.imshow(imagenumpy[i - 1])
            plt.show()
            return
