...
save_path = './icCNN/basic_fmap/resnet/'  # for get_feature
acc_path = './icCNN/basic_fmap/resnet/acc/'
...
def net_train():
    ...
    device = torch.device("cuda")
    if LAYERS == '50':
        net = ResNet50(NUM_CLASSES, pretrained=True).cuda()
    else:
        net = ResNet18(NUM_CLASSES, pretrained=True).cuda()
...
def get_feature():
    ...
    if LAYERS == '50':
        net = ResNet50(NUM_CLASSES).cuda()
    else:
        net = ResNet18(NUM_CLASSES).cuda()
    print(net)
    sys.exit()

    #net = nn.DataParallel(net).to(device)
    test = test_celeb if DATANAME=='celeb' else test_orir
