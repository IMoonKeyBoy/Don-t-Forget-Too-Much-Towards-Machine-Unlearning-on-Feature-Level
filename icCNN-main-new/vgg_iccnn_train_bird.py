...
celeb_file = '../../datasets/CelebA'

save_path = './icCNN/basic_fmap/vgg_iccnn_original/'
acc_path = './icCNN/basic_fmap/vgg_iccnn_original/acc/'
log_path = './icCNN/vgg/'

dataset = '%s_vgg_%s_vgg_iccnn_original' % (LAYERS, DATANAME)
log_path = log_path + dataset + '/'
pretrain_model = "/home/hengxu/Data/PycharmWorkspace/Technical Paper 3/author_model_from_Feishu/vgg13/voc_brid/model_2500.pth"
...
def net_train():
    ...
    net = None
    if LAYERS == '13':
        net = vgg16(arch='vgg13_bn', cfg='B', num_class=NUM_CLASSES, device=device, pretrained=True, progress=True).cuda()
    elif LAYERS == '16':
        net = vgg16(arch='vgg16_bn', cfg='D', num_class=NUM_CLASSES, device=device, pretrained=True, progress=True).cuda()
    ...
    for epoch in range(EPOCH + 1):
        ...
        if (epoch) % T == 0 and epoch < STOP_CLUSTERING:
            with torch.no_grad():
                Ground_true, loss_mask_num, loss_mask_den = offline_spectral_cluster(net, trainset_loader, DATANAME)     #torch.Size([512, 512]) torch.Size([5, 512, 512]) torch.Size([5, 512, 512])
            ...
            for batch_step, input_data in tqdm(enumerate(trainset_loader, 0), total=len(trainset_loader), smoothing=0.9):
                ...
                if DATANAME != 'celeb':
                    ...
                    labels = None
                loss_ = cs_loss.update(corre, loss_mask_num, loss_mask_den, labels)  # torch.Size([32, 512, 512]) torch.Size([5, 512, 512]) torch.Size([5, 512, 512]) torch.Size([32])
...
def test_ori(net, testdata,NUM_CLASSES):
    correct, total = .0, .0
